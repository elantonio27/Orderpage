...
/*  Promse.all([new Promise(), ]).then()*/
  responseJSON=postData(url='http://localhost:1337/longdrinklist',jsonArray); //Verbindung mit Server aufbauen und die drinkliste speichern
...
fetch ("http://localhost:1337/longdrinklist").then (function (response) {
   return response.json();
}).then (function (data) {
   appendData (data);
}).catch (function (error) {
